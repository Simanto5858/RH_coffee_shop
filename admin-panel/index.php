...
$user_result = mysqli_query($conn, $user_query) or die("Query Unsuccessful");
if (mysqli_num_rows($user_result) > 0) {
  $users_count = mysqli_fetch_assoc($user_result);
}
// messages
$contact_message_query = "SELECT COUNT(*) AS count FROM contact_messages";
$contact_message_result = mysqli_query($conn, $contact_message_query) or die("Query Unsuccessful");
if (mysqli_num_rows($contact_message_result) > 0) {
  $contact_messages_count = mysqli_fetch_assoc($contact_message_result);
}
?>
    ...
    <div class="col-md-3">
      <div class="card">
        <div class="card-body">
          ...
          <p class="card-text">number of users: <?php echo $users_count['count']; ?></p>
        </div>
      </div>
    </div>
    <div class="col-md-3">
      <div class="card">
        <div class="card-body">
          ...
          <p class="card-text">number of contact_messages: <?php echo $contact_messages_count['count']; ?></p>
        </div>
      </div>
    </div>
  </div>
